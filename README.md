...
- [doc/task.md](doc/task.md) — описание задачи, данных, модели, валидации и артефактов
- [doc/grade.md](doc/grade.md) — ожидаемые баллы по каждому пункту задания
...
(каждый час) эмулирует появление нового батча потоковых данных — за 10 часов
проходят все 10 батчей. Модель и состояние пайплайна сохраняются между
запусками через кэш GitHub Actions.

### Архитектура

```
push / pull_request / CRON (каждый час) / ручной запуск
              |
              v
         [test]  — pytest tests/
              |
